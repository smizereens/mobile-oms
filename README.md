...
В системе предусмотрено три роли, хранящиеся в поле `role` модели `User`:

| Роль       | Описание |
|------------|----------|
| `employee` | Базовая роль по умолчанию |
| `manager`  | Расширенный доступ к заказам |
| `admin`    | Административный доступ |

> Разграничение прав по ролям на уровне API-маршрутов запланировано в рамках доработки — см. раздел «Возможные доработки».
...
```text
GET  /api/orders              # список заказов (фильтрация по ?status= и ?search=)
...
COMPLETED  → ARCHIVED
CANCELLED  → ARCHIVED
ARCHIVED   → (терминальный, изменение невозможно)
```

> Архивация выполняется через `PUT /api/orders/:id/status` с телом `{ "status": "ARCHIVED" }` — отдельного endpoint не предусмотрено.
...
```bash
git clone https://github.com/smizereens/mobile-oms.git
cd mobile-oms
```
